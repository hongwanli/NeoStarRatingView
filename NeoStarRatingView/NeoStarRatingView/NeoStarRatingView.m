...
@implementation NeoStarRatingView
...
- (id)initWithFrame:(CGRect)frame numberOfStar:(int)number
{
    ...
    if (self) {
        ...
        self.starBackgroundView = [self buidlStarViewWithImageName:kGray_Star];
        self.starForegroundView = [self buidlStarViewWithImageName:kHL_Star];
...
- (void)setScore:(float)score withAnimation:(BOOL)isAnimate completion:(void (^)(BOOL finished))completion
{
    ...
    if(isAnimate)
    {
        __weak NeoStarRatingView * weekSelf = self;
...
- (UIView *)buidlStarViewWithImageName:(NSString *)imageName
{
    ...
    for (int i = 0; i < self.numberOfStar; i ++)
    {
        ...
        imageView.frame = CGRectMake(i * 24 + i * kGap_Star, 0, 24, frame.size.height);
